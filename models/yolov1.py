...
from models.utils import get_iou, get_max_iou_indices, get_aps
...
class YOLOv1(Module):
    ...
    def get_loss(
        ...
        obj_mask_batch = torch.ne(grid_cell_iou_batch, 0)
        obj_mask_batch = (
            obj_mask_batch *
            mask_batch.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        )
    ...
    def collate_fn(self, batch, pad_val=-1):
    ...
    def collate_fn_backup(self, batch):
        augmenter = iaa.Sequential([
            iaa.Affine(
                scale=(0.8, 1.2),
                translate_percent={"x": (-0.2, 0.2), "y": (-0.2, 0.2)},
